...
#undef ON_ALL_ACTIONS
    ...
    safe_void_coroutine TerminalPage::_OpenHyperlinkHandler(const IInspectable sender, const Microsoft::Terminal::Control::OpenHyperlinkEventArgs eventArgs)
    {
        try
        {
            ...
            // in the path, so a link ending in ...routes.yaml#L6-L7 resolves to a file that
            // does not exist and comes back as "this link is invalid". Drop it here, ahead
            // of the checks below, so that _IsUriConsideredSomewhatSafe weighs the path that
            // is actually going to be opened rather than one with a suffix stuck on the end.
            if (parsed.SchemeName() == L"file" && !parsed.Fragment().empty())
            {
                uriString = winrt::hstring{ Utils::StripUriFragment(uriString) };
                parsed = winrt::Windows::Foundation::Uri(uriString);
            }
    ...
    // Resolves an Lxss registration GUID to the distro's name. That GUID is both the
    // subkey name under Lxss and the value wsl.exe takes as --distribution-id, so this
    // one lookup answers "the default distro" and "the distro this profile launches".
    std::wstring TerminalPage::_GetWslDistroById(const std::wstring& distroId)
    {
    ...
    std::wstring TerminalPage::_GetDefaultWslDistro()
    {
        ...
        return defaultGuid.empty() ? std::wstring{} : _GetWslDistroById(defaultGuid);
    }

    // ...\ubuntu.exe -> "ubuntu". Lowercased and stripped of the separators distro names
    // are written with, so that a launcher's name can be compared against a registration's:
    ...
    // Matches a launcher against the registered distros: ubuntu.exe -> "Ubuntu". A distro's
    // own Store package contributes a Terminal profile that names nothing but its launcher
    // -- Ubuntu's fragment is `"commandline": "ubuntu.exe"` -- with no -d to parse and no WSL
    // path translation style declared, so the name is the only tell that profile has. It is
    // also a better answer than the default distro, which would only be right by luck.
    std::wstring TerminalPage::_GetWslDistroMatchingLauncher(std::wstring_view exePath)
    {
    ...
    {
        const auto termControl = sender.try_as<TermControl>();
        if (!termControl)
        {
            return {};
        }

        const auto settings = termControl.Settings();
        if (!settings)
        {
            return {};
        }
        ...
        if (const std::wstring cmd{ settings.Commandline() }; !cmd.empty())
        {
        ...
        const auto launcherDistro = _GetWslDistroMatchingLauncher(exePath);

        // Is this pane WSL at all? Only then may a bare POSIX path be read as one of its
        // own. Ungated, the default distro gets guessed for every pane, so a Windows
        // program printing file:///etc/hosts into a PowerShell tab would silently open
        // \\wsl.localhost\<default distro>\etc\hosts. Drive letters, /mnt/<letter>/ and
        // explicit \\wsl.localhost\ UNC paths are unaffected either way -- they resolve
        // without needing to know the distro at all.
        //
        // Three tells, because no single one covers every WSL profile: the generator and
        // the fragments WSL itself ships declare the path translation style, a distro's
        // own Store package contributes a profile that only names its launcher, and a
        // hand-written profile may just invoke wsl.exe.
        ...
        for (int i = 0; i < argc; ++i)
        {
            ...
            // The profile fragments WSL ships for store distros launch by id rather than
            // by name: `wsl.exe --distribution-id {guid} --cd ~`. Without this arm they
            // fall through to the default distro, so a link clicked in a Debian pane
            // opens the same path under Ubuntu instead.
            else if (arg == L"--distribution-id")
            {
                if (i + 1 < argc)
                {
                    if (auto byId = _GetWslDistroById(std::wstring{ argv.get()[i + 1] }); !byId.empty())
                    {
                        return byId;
                    }
                }
            ...
            else if (til::starts_with_insensitive_ascii(arg, L"--distribution-id="))
            {
                ...
                if (auto byId = _GetWslDistroById(std::wstring{ arg.substr(eqPos + 1) }); !byId.empty())
                {
                    return byId;
                }
            }
        ...
        return launcherDistro.empty() ? _GetDefaultWslDistro() : launcherDistro;
    }
